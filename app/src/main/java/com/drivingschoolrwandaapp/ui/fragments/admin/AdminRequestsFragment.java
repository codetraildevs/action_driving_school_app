...
import com.drivingschoolrwandaapp.ui.adapters.AdminRequestAdapter;
...
public class AdminRequestsFragment extends Fragment {
    ...
    private List<AdminRequest> allRequests = Collections.emptyList();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.retry_button).setOnClickListener(v -> adminViewModel.refreshRequests());
        ...
        swipeRefreshLayout.setOnRefreshListener(() -> {
            ...
            adminViewModel.refreshRequests();
        });
        ...
        adminViewModel.getRequests().observe(getViewLifecycleOwner(), resource -> {
            if (resource.getStatus() == Resource.Status.LOADING) {
                ...
                swipeRefreshLayout.setRefreshing(false);
                errorContainer.setVisibility(View.GONE);
                allRequests = resource.getData() != null ? resource.getData() : Collections.emptyList();
                applyFilter();
            }
        });

        if (adminViewModel.getRequests().getValue() == null) {
            adminViewModel.refreshRequests();
        }
    }

    private void applyFilter() {
        List<AdminRequest> filtered = AdminListFilter.filterRequests(allRequests, query);
        if (filtered.isEmpty()) {
            recyclerView.setVisibility(View.GONE);
            emptyState.setVisibility(View.VISIBLE);
            emptyStateText.setText(query.trim().isEmpty()
                    ? R.string.admin_empty_requests
                    : R.string.admin_no_matching_requests);
        } else {
